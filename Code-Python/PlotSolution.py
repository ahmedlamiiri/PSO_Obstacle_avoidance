...
class PlotSolution:
    ...
    def smooth_path(self, points, num_points=100):
        # Get the number of points in the original path
        npts = points.shape[1]
        
        # Create a parameter array for the spline
    ...
    def plot_solution(self):
        ...
        xs, ys, zs = self.model['start']
        xf, yf, zf = self.model['end']
        threats = self.model.get('threats', [])
        ...
        ax.plot_surface(X, Y, H, cmap='terrain', alpha=0.5)
        ...
        if threats:
            for threat in threats:
                x, y, z, radius = threat
                height = 250  # Hauteur fixe des cylindres
                
                # Create sphere/cylinder for threat visualization
        ...
        ax_top.contour(X, Y, H, levels=20, cmap='terrain')
        ...
        # Plot threats in side view
        if threats:
            for threat in threats:
                x, y, z, radius = threat
                height = 250  # Hauteur fixe des cylindres
                rect = plt.Rectangle((x - radius, z), 2*radius, height, color='red', alpha=0.3)
...
if __name__ == "__main__":
    # Create test model with real terrain data
    from PIL import Image
    import numpy as np
    
    # Load the terrain data
    terrain = np.array(Image.open('ChrismasTerrain.tif'))
    terrain[terrain < 0] = 0
    
    # Create coordinate grids
    Y, X = np.mgrid[0:terrain.shape[0], 0:terrain.shape[1]]
    ...
    # Create test solution (a simple path)
